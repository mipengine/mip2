...
let supportHistory = window.history && 'state' in window.history
...
function initHistoryStateStack () {
  ...
  if (window.history.length > 1) {
    try {
      let historyState = JSON.parse(window.localStorage.getItem(MIP_HISTORY_STATE_STACK_LOCAL_KEY))
      if (historyState && historyState.length) {
        // 为了有效控制localStorage大小，每次读取时应该只读取不大于当前tab历史条目长度
        // 因为大于历史条目长度之前的记录都是过期的state，无需读取
        HISTORY_STATE_STACK = historyState.slice(-window.history.length)
      }
    } catch (err) {
...
function initHistoryArrayStack (routerBase) {
  let firstPageFullPath = window.location.href.replace(window.location.origin + routerBase, '/')

  try {
    // 如果localStorage中有历史访问记录，且最新一条和当前访问的是同一个页面
    // 那应该把之前的记录也加进来，主要解决在访问过程中刷新导致history列表丢失的问题
    let historyStack = JSON.parse(window.localStorage.getItem(MIP_HISTORY_ARRAY_STACK_LOCAL_KEY))
    if (
      historyStack &&
            historyStack.length &&
