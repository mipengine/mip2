...
describe('page.util', function () {
  ...
  describe('ease-scroll', function () {
    ...
    it('scroll to element with position', async () => {
      ...
      await timer.sleep(300)
      expect(viewport.getScrollTop()).to.be.equal(Math.floor(top+100-viewport.getHeight()/2))
      handleScrollTo(target, {duration: 200, position: 'bottom'})
      await timer.sleep(300)
      expect(viewport.getScrollTop()).to.be.equal(Math.floor(top+200-viewport.getHeight()))
